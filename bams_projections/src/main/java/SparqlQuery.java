...
public class SparqlQuery
{
	 ...
	 * @see MultiHashMap
	 */
	public HashMap<Integer,brainRegionSynonyms> runSelectQueryNeurolex() {
		...
		try {
			...
			return parseResultNeurolex(queryResult);
	 ...
	 * @throws Exception
	 */
	private HashMap<Integer,brainRegionSynonyms> parseResultNeurolex(InputStream queryResult) 
	throws Exception {
	...
	public MultiHashMap<Integer, NeurolexPageId> runSelectQueryBAMSProjections() {
		String queryString = getComposedQuery();
		...
		try {
			...
			return parseSPARQLResultBAMSProjections(queryResult);
	 ...
	 * @see MultiHashMap
	 */
	public HashMap<Integer,NeurolexPageId> runSelectQueryNeurolexHashCode() {
		String queryString = getComposedQuery();
		try {
			// URL encode query string
			queryString = URLEncoder.encode(queryString, "UTF-8");
	...
	private MultiHashMap<Integer, NeurolexPageId> parseSPARQLResultBAMSProjections(
			InputStream queryResult) throws Exception {
		MultiHashMap<Integer,NeurolexPageId> vec = new MultiHashMap<Integer,NeurolexPageId>();
		Pattern p = Pattern.compile("[\\-][0-9]");
		
		String brainRegionA = null;
		String brainRegionAName = null;
		String brainRegionB = null;
		String brainRegionBName = null;
			...
			if (event == XMLStreamConstants.START_ELEMENT) {
				if ("binding".equals(parser.getLocalName())) {
					...
					if (selectedVariable != null) {
						...
						if(selectedVariable.equals("$brainRegionA"))
							brainRegionA = elementText;	
						else if(selectedVariable.equals("$brainRegionAName"))
							brainRegionAName = elementText.toLowerCase().replace(",", "");
						else if(selectedVariable.equals("$brainRegionB"))
							brainRegionB = elementText;	
						else if(selectedVariable.equals("$brainRegionBName"))
							brainRegionBName = elementText.toLowerCase().replace(",", "");
						if(brainRegionA != null && brainRegionAName != null && brainRegionB != null && 
								brainRegionBName != null){
							vec.put(brainRegionA.hashCode(), new NeurolexPageId(brainRegionA,brainRegionAName,brainRegionBName,
									brainRegionB));
							//vec.put(brainRegionB.hashCode(), new NeurolexPageId(brainRegionB,brainRegionBName,brainRegionAName,
								//	brainRegionA));
							brainRegionA = null;
							brainRegionAName = null;
							brainRegionB = null;
							brainRegionBName = null;
						}
					}
	 ...
	 * @throws Exception
	 */
	private HashMap<Integer,NeurolexPageId> parseSPARQLResultNeurolexHashCode(InputStream queryResult) 
	throws Exception {
					...
					if ("binding".equals(parser.getLocalName())) {
						...
						if (selectedVariable != null) {
							...
							if(selectedVariable.equals("$name")){
								...
								elementUri = elementText;
							}	

							if(elementName != null && elementUri != null){
								resultMap.put(elementUri.hashCode(), new NeurolexPageId(elementUri,elementName));
