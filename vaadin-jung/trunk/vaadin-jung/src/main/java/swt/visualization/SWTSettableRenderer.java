 ...
 */
public class SWTSettableRenderer extends SWTAbstractRenderer {
     ...
     * @param el
     */
    public SWTSettableRenderer(StringLabeller sl, EdgeStringer el) {
        this.mStringLabeller = sl;
    ...
    public void paintEdge(AwtG2DWrapper g2d, Edge e, int x1, int y1, int x2, int y2) {
        ...
        if (edgeColorFunction == null) {
            ...
            g2d.setColor(edgeColorFunction.getEdgeColor(e));
        }

        if (edgeWidth == 1)
            drawEdgeSimple(g2d, e, x1, y1, x2, y2);
        else
            drawEdge(edgeWidth, g2d, e, x1, y1, x2, y2);
        ...
        String label = (mEdgeLabelFunction == null) ? null : mEdgeLabelFunction.getLabel(e);
        if (label != null) {
     ...
     * @param y2
     */
    public void labelEdge(AwtG2DWrapper g2d, Edge e, String label, int x1, int x2,
            int y1, int y2) {
     ...
     * xend, yend
     */
    protected void drawArrowhead(AwtG2DWrapper g2d, Edge e, int xsource, int ysource, int xdest, int ydest) {
     ...
     */
    protected void drawEdge(float edgeWidth, AwtG2DWrapper g2d, Edge e, int x1,
...
// caches BasicStroke:
//    Stroke[] strokeTable = new Stroke[10];

    protected void drawEdgeSimple(AwtG2DWrapper g, Edge e, int x1, int y1, int x2,
            int y2) {
     ...
     * @return
     */
//    protected boolean isSelfLoop(Edge e) {
...
//    }
    ...
    protected String getLabel(Vertex v) {
        if (mStringLabeller == null) return ""; //$NON-NLS-1$
     ...
     * @param y
     */
    protected void paintLightVertex(AwtG2DWrapper g, Vertex v, int x, int y, String label) {
        Color bg;
        if (vertexColorFunction == null) {
            if (isPicked(v)) {
        ...
        if (label.equals("?")) return; //$NON-NLS-1$
        g.setColor(fg);
